...
minerdream_copperarrow.on_step = function(self, dtime)
	if not self.timer_to_die then self.timer_to_die = 0 end
	self.timer_to_die = dtime + self.timer_to_die
	if self.timer_to_die >= 5 then
		self.object:remove()
		return
	end
...
minerdream_water_ray.on_step = function(self, dtime)
	...
	if self.lastpos.x ~= nil then
		if node and node.name and minetest.registered_nodes[node.name].walkable then
			if not minetest.setting_getbool("creative_mode") then
				minetest.add_item(self.lastpos, "")
			end
			minetest.sound_play("default_dig_cracky", {pos = self.lastpos, gain = 0.8})

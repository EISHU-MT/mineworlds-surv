...
rs = minetest.get_translator(minetest.get_current_modname())
...
function ranks.get_rank(name)
	...
	local STFrank = storage:get_string("stf:"..name)
	local NRLrank = storage:get_string("nrl:"..name)

	--if rank ~= "" and registered[rank] then
	--	return rank
	--end
	if STFrank ~= "" and NRLrank ~= "" then
		return {
			staff=STFrank,
			normal=NRLrank,
		}
...
function ranks.update_privs(name, trigger)
	...
	local rankt = ranks.get_rank(name)
	if rankt ~= nil then
		if type(rankt) == "table" then
			local dont_continue = false
			for rn, rank in pairs(rankt) do
				-- [local function] Warn
...
function ranks.set_rank(name, rank)
	...
	if registered[rank] and minetest.player_exists(name) then
		...
		if registered[rank].staff then
			storage:set_string("stf:"..name, rank)
		else
			storage:set_string("nrl:"..name, rank)
		end
...
function ranks.remove_rank(name)
	...
	if rank ~= nil then
		...
		if type(rank) == "table" then
			for rn, r in pairs(rank) do
				if registered[r] then
					if registered[r].staff then
						storage:set_string("stf:"..name, nil)
					else
						if registered[r].prefix ~= "Miembro" then
							storage:set_string("nrl:"..name, "miembro")
						end
					end
				end
			end
			...
			if registered[rank] then
				if registered[rank].staff then
					...
					if registered[rank].prefix ~= "Miembro" then
						storage:set_string("nrl:"..name, "miembro")
					end
				end
			end
	...
	if minetest.settings:get("ranks.prefix_chat") ~= "false" then
		...
		if rank ~= nil then
			...
			if def.prefix then
            	...
            	dc.send({content = def.prefix.." ".."<**" .. name .. "**>  " .. message})
                return true
			end
		end
	end
    dc.send({content = "<**" .. name .. "**>  " .. message})
end

ranks.default = "miembro"
...
minetest.register_chatcommand("getrank", {
	...
	func = function(name, param)
		if param and param ~= "" then
			local r = ranks.get_rank(name)
			if not (type(r) == "table") then r = {staff=nil,normal=r} end
			local rank = r.staff or r.normal or "No rank"
			if rank then
				if rank ~= "No rank" then
